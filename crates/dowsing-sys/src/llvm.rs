...
/// Raw LLVM profile counter runtime.
#[derive(Debug, Clone)]
...
/// Result of starting LLVM profile counter capture.
#[derive(Debug)]
pub enum LlvmStart {
    /// Capture started and must be finished or discarded.
    Started(LlvmSession),

    /// LLVM profile counters are currently owned by another session.
    Busy,
}

/// Raw LLVM profile feedback observed during one execution.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct LlvmFeedback {
    /// Counter coverage features.
    pub features: Vec<RawFeature>,

    /// Coarse execution intensity derived from hit-count buckets.
    pub hit_count_weight: u64,
}

/// Active LLVM profile counter capture session.
#[derive(Debug)]
...
impl LlvmRuntime {
    ...
    pub fn new() -> crate::Result<Self> {
        ...
        if counter_count <= 0 {
            return Err(Error::new("LLVM coverage runtime reported no counters"));
        }
        Ok(Self {
            runtime,
    ...
    /// Start an exclusive profile counter capture.
    pub fn start(&self) -> LlvmStart {
        let Some(guard) = try_lock_capture() else {
            return LlvmStart::Busy;
        };
        self.runtime.reset_counters();
        LlvmStart::Started(LlvmSession {
            guard,
    ...
    /// Reset the process-wide LLVM profile counters.
    pub fn reset_counters() -> crate::Result<()> {
        let _guard = wait_for_capture_lock();
        ProfileRuntime::new()?.reset_counters();
        Ok(())
    }
}
...
impl LlvmSession {
    ...
    pub fn finish(self, bucketing: bool) -> crate::Result<LlvmFeedback> {
        let feedback = counter_coverage(self.runtime, self.counter_count, bucketing)?;
        drop(self.guard);
        Ok(feedback)
    }
}
...
impl ProfileRuntime {
    fn new() -> crate::Result<Self> {
        ...
        {
            Err(Error::new(
                "LLVM coverage runtime is unavailable under Miri",
            ))
        }
    ...
    fn counter_bounds(self) -> crate::Result<(*const u64, *const u64)> {
        ...
        if begin.is_null() || end.is_null() {
            return Err(Error::new(
                "LLVM coverage runtime returned null counter bounds",
            ));
        }
        Ok((begin, end))
    }
...
fn load_symbol<F>(symbol: &'static CStr) -> crate::Result<F>
where
    F: Copy,
{
...
fn counter_coverage(
    ...
    for index in 0..counter_count {
        ...
        features.push(RawFeature::new(id));
...
mod tests {
    ...
    #[test]
    fn llvm_runtime_is_unavailable_under_miri() {
        assert!(LlvmRuntime::new().is_err());
        assert!(LlvmRuntime::reset_counters().is_err());
    }
}
