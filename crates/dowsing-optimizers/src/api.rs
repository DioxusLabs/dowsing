...
use dowsing_core::{
    CoverageCapture, CoverageId, CoverageSet, MutationSource, NoCoverage, ParallelCoverageCapture,
};
...
impl<Capture: CoverageCapture> Engine<Capture> {
    ...
    pub(super) fn with_mutation_sources(
        self,
        sources: impl IntoIterator<Item = MutationSource>,
    ) -> Self {
    ...
    pub fn test_corpus_energies(&self) -> Vec<f64> {
        self.shared
            .lock()
            .corpus
            .iter()
    ...
    pub fn test_feature_frequency(&self, id: CoverageId) -> u64 {
        *self.shared.lock().coverage_frequency.get(&id).unwrap_or(&0)
    }
    pub fn test_corpus_path_lens(&self) -> Vec<usize> {
        self.shared
            .lock()
            .corpus
            .iter()
    ...
    pub fn test_best_path_score(&self) -> Option<(usize, u64, usize)> {
        self.shared
            .lock()
            .corpus
            .iter()
    ...
    pub fn test_best_case_cost(&self) -> Option<super::prelude::CaseCost> {
        self.shared
            .lock()
            .min_path_best
            .map(|score| score.case_cost)
    }
    pub fn test_best_nonzero_bytes(&self) -> Option<usize> {
        self.shared
            .lock()
            .min_path_best
            .map(|score| score.nonzero_bytes)
    }
    pub fn test_best_prefix(&self) -> Option<Vec<u8>> {
        let state = self.shared.lock();
        let index = state.min_path_best_index?;
        Some(state.corpus.get(index)?.prefix.clone())
    }
    pub fn test_dictionary_values(&self) -> Vec<Vec<u8>> {
        (*self.shared.lock().dictionary).clone()
    }
    pub fn test_reducer_feedback(&self) -> (u64, u64, Vec<u16>, bool) {
        let state = self.shared.lock();
        (
    ...
    Capture: CoverageCapture,
{
    ...
    pub fn with_mutations(self, sources: impl IntoIterator<Item = MutationSource>) -> Self {
        Self {
...
pub struct ParallelCases<Capture: ParallelCoverageCapture = NoCoverage> {
