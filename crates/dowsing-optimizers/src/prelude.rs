pub(super) use super::optimize::GoalBehavior;
pub(super) use dowsing_core::{
    Case, CoverageCapture, CoverageId, CoverageSet, MutationSource, MutationWeights, NoCoverage,
    ReductionOp,
};
...
pub fn curious() -> Curious<NoCoverage> {
    curious_with(NoCoverage)
}

/// Build a coverage-maximizing RNG iterator with an explicit backend.
pub fn curious_with<Capture: CoverageCapture>(capture: Capture) -> Curious<Capture> {
    Curious {
        engine: Engine::new(capture, super::optimize::goals::MaximizeCoverage),
    }
}
...
pub fn cautious() -> Cautious<NoCoverage> {
    cautious_with(NoCoverage)
}

/// Build a code-path-minimizing RNG iterator with an explicit backend.
pub fn cautious_with<Capture: CoverageCapture>(capture: Capture) -> Cautious<Capture> {
    Cautious {
        engine: Engine::new(capture, super::optimize::goals::MinimizeCoverage),
    }
}

/// Build a generalized optimizer for `goal`.
pub fn optimize<G>(goal: G) -> super::optimize::Optimizer<G, NoCoverage>
where
    G: super::optimize::Goal,
{
    optimize_with(goal, NoCoverage)
}

/// Build a generalized optimizer for `goal` with an explicit backend.
pub fn optimize_with<G, Capture>(
    goal: G,
    capture: Capture,
) -> super::optimize::Optimizer<G, Capture>
where
    G: super::optimize::Goal,
    Capture: CoverageCapture,
{
    super::optimize::optimizer_from_engine(Engine::new(capture, goal))
}

/// Coverage-maximizing search returned by [`curious`].
pub struct Curious<Capture: CoverageCapture = NoCoverage> {
    pub(super) engine: Engine<Capture>,
}

/// Code-path-minimizing search returned by [`cautious`].
pub struct Cautious<Capture: CoverageCapture = NoCoverage> {
    pub(super) engine: Engine<Capture>,
}

pub(super) struct Engine<Capture: CoverageCapture = NoCoverage> {
