...
fn main() {
    ...
    if let Some(n) = std::env::args()
        .nth(2)
        .filter(|_| std::env::args().nth(1).as_deref() == Some("bench"))
    {
        ...
        println!(
            "bench: {n} reads, virtual elapsed {:.6}s (acc {acc})",
